            ...
            viewModel.aiResponseFlow.collect { response ->
                when (response) {
                    ...
                    is TextStreamResponse.Error -> {
                        requestEnd()
                        LoadScreenAnimUtil.instance.loadScreenAnim(ScenarioSealed.Error)
                        ...
                        if (isNetworkError) {
                            Toast.makeText(this@MainActivityWeLoActivity, "网络连接失败，请检查 Wi-Fi 或数据网络", Toast.LENGTH_LONG).show()
                        } else {
