...
class AudioPlayerUtil(private val ttsService: TtsService) {
    ...
    private val audioBlendShapeMap: MutableMap<Int, AudioBlendShapeData> = mutableMapOf()
...
//    private val ttsService: TtsService = activity.getTtsService()
    ...
    // 保留音频片段获取逻辑（仅处理音频）
    private suspend fun getNextAudioBlendShape(): AudioBlendShapeData {
    ...
    fun playText(text: String, id: Int, isLast: Boolean) {
        ...
        // 音频数据对象（移除动画相关参数）
        val audioBlendShape = AudioBlendShapeData(
...
data class AudioBlendShapeData(
    val id: Int,
    val is_last: Boolean,
    val text: String,
